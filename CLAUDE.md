...
1. **プロジェクト構造**: コードを論理的なモジュールに整理：
   - `/src` - メインソースコード（単体テストを隣接配置）
   - `/lib` - 再利用可能なユーティリティ
   - `/tests` - E2Eテストファイル（プロジェクト全体のワークフロー）
   - `/config` - 設定ファイル
...
6. **テスト配置ルール**: テストファイルは以下の規則に従って配置する：

### テスト配置の基本原則

- **単体テスト**: `src/` 配下でソースファイルの隣接配置
- **統合テスト**: `src/` 配下で `__tests__` ディレクトリ
- **E2Eテスト**: プロジェクトルートの `tests/` ディレクトリ

### 判断基準

| テスト種別 | 配置場所 | 例 |
|-----------|---------|---|
| **単体テスト** | `src/module.test.ts` | `src/database/repositories/project.test.ts` |
| **統合テスト** | `src/path/__tests__/module.test.ts` | `src/api/__tests__/gitlab-client.test.ts` |
| **共通テストユーティリティ** | `src/path/__tests__/utilities.ts` | `src/database/__tests__/setup.ts` |
| **E2Eテスト** | `tests/category/scenario.test.ts` | `tests/e2e/full-sync.test.ts` |

### 配置基準
- **単一モジュールの機能** → ソースファイル隣接配置
- **複数モジュールの連携** → `__tests__` ディレクトリ
- **アプリケーション全体のワークフロー** → プロジェクトルートの `tests/` ディレクトリ

### 重要な注意点
- **環境依存性の有無はテスト種別を決定しない** - データベースを使う単体テストも隣接配置
- **共通テストユーティリティ**は該当モジュールの `__tests__` ディレクトリに配置
- **インポートパス**は配置場所に応じて相対パスを使用
